...
規格見 [SPEC.md](SPEC.md)（v0.6），子彈時間設計案見 docs/bullet_time_dlc.md。
...
開場為 **launcher 教學頁**：說明核心循環、集氣、泡泡顏色獎勵，選「簡單／中等／最難」即開始；遊戲中 `M` 隨時回教學頁。
...
- **按住 `L` = 子彈時間**：世界慢放、場景擠出深度（平台一格變三格）、**W/S 前後移動紙片閃敵**（離面碰不到也打不到、更耗能）、空白照常跳。
  熟成與掙脫**不變慢**（慢下來≠安全）。能量上限 5s，只靠連鎖回充（單發不給、連越長給越多）；
  簡單難度開局送 3s 且主角不被慢放。左上藍條顯示能量。
...
玩家小龍由 `asset_raw/dragon_ref.png` 概念圖 **pixel-by-pixel 轉檔**而來（`tools/convert_ref.mjs`：
去背 flood-fill＋透明洞補白＋區塊眾數取樣＋調色盤合併 → 32×32 兩幀）。敵人為 `tools/pixelart.py` 自繪，
暴走型態為紅色調色盤置換。新概念圖丟進 `asset_raw/` 跑一次轉檔器即可換角。
